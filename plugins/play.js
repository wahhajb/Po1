...
thumbnail: await (await fetch(vid.thumbnail)).buffer()}}}, { quoted: m })
    ...
    let q = '360p'
	const texto1 = `يـؤتـيؤب ⛈️ ألأغنيه\n
	👑 *العنوان ∙* ${vid.title}\n
        ⚡ *المده ∙* ${vid.timestamp}\n
        🪐 *المشاهده ∙* ${vid.views}\n
        🦈 *الفنان ∙* ${vid.author.name}\n
...
if (size.split('MB')[0] >= limit) return conn.reply(m.chat,` الملف أكثر من ${limit} ميغابايت، تم إلغاء التنزيلa.`,  m, fake,).then(_ => m.react('✖️'))
